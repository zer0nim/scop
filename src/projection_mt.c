...
/*   Updated: 2019/08/15 20:36:25 by emarin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
, float z_far)
{
	...
	res->cont[2 * 4 + 3] = -1.0f;
	res->cont[3 * 4 + 2] = -(2.0f * z_far * z_near) / (z_far - z_near);
	return (res);
}
...
t_matrix	*mt_orthographic(t_vect4 l_r_b_t, float z_near, float z_far)
{
	...
	res->cont[3 * 4 + 0] = -(right + left) / (right - left);
	res->cont[3 * 4 + 1] = -(top + bottom) / (top - bottom);
	res->cont[3 * 4 + 2] = -(z_far + z_near) / (z_far - z_near);
	return (res);
}
